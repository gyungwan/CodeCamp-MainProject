  ...
  await Promise.all([
    new Promise((resolve, reject) => {
      storage
        ...
        .pipe(storage.file(`thumb/${event.name}`).createReadStream()) //6. 생선된 썸네일을 재업로드
        .on("finish", () => resolve())
        .on("error", () => reject());
    }),
    new Promise((resolve, reject) => {
      storage
        ...
        .createReadStream() // 4. 기존의 파일을 읽어오기
        //5. event 안에 있는 file 을  활용하여 썸네일 생성
        .pipe(sharp().resize({ wjdth: 640 })) //가로길이에 맞게끔 비율이 자동으로  만든 썸네일을 버킷으로 업로드
        .pipe(storage.file(`thumb/${event.name}`).createReadStream()) //6. 생선된 썸네일을 재업로드
        .on("finish", () => resolve())
        .on("error", () => reject());
    }),
    await new Promise((resolve, reject) => {
      storage
        ...
        .createReadStream() // 4. 기존의 파일을 읽어오기
        //5. event 안에 있는 file 을  활용하여 썸네일 생성
        .pipe(sharp().resize({ wjdth: 1280 })) //가로길이에 맞게끔 비율이 자동으로  만든 썸네일을 버킷으로 업로드
        .pipe(storage.file(`thumb/${event.name}`).createReadStream()) //6. 생선된 썸네일을 재업로드
        .on("finish", () => resolve())
        .on("error", () => reject());
    }),
